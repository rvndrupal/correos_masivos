...
switch($radio){
case 0;
$con = oci_parse($conn,"SELECT * FROM VWN_DIRECTORIO_COADYUVANTES_PM WHERE SIGLAS ='".$sigla."' AND ACRONIMO ='".$acronimo."' AND upper(RAZON_SOCIAL||''||UNIDAD_ORGANIZACIONAL||' '||SIGLAS||' '||TIPO_ORGANO_COADYUVANCIA||' '||CAMPO_ESTUDIO||' '||ENTIDAD||' '||CLAVE_CEDULA_AUTORIZACION||' '||CORREO_ELECTRONICO||' '||RFCPARTICIPANTE)like upper('%".$nombre."%')");
break;
case 1;
$con = oci_parse($conn,"SELECT * FROM VWN_DIRECTORIO_COADYUVANTES_PM WHERE SIGLAS ='".$sigla."' AND ACRONIMO ='".$acronimo."' AND upper(RAZON_SOCIAL)like upper('%".$nombre."%')");
break;
case 2;
$con = oci_parse($conn,"SELECT * FROM VWN_DIRECTORIO_COADYUVANTES_PM WHERE SIGLAS ='".$sigla."' AND ACRONIMO ='".$acronimo."' AND upper(CLAVE_CEDULA_AUTORIZACION)like upper('%".$nombre."%')");
break;
case 3;
$con = oci_parse($conn,"SELECT * FROM VWN_DIRECTORIO_COADYUVANTES_PM WHERE SIGLAS ='".$sigla."' AND ACRONIMO ='".$acronimo."' AND upper(CORREO_ELECTRONICO)like upper('%".$nombre."%')");
break;
case 4;
$con = oci_parse($conn,"SELECT * FROM VWN_DIRECTORIO_COADYUVANTES_PM WHERE SIGLAS ='".$sigla."' AND ACRONIMO ='".$acronimo."' AND upper(RFCPARTICIPANTE)like upper('%".$nombre."%')");
break;
}
...
echo"</tr>";
echo"<tr>";
echo"<th scope='row' bgcolor='#A4A4A4'>&raquo;$nombre &raquo;$sigla &raquo;$acronimo</th>";
echo"</tr>";
echo"<tr>";
...
?>
 ...
 echo "<tr>";
  ...
  //   }//else
   if(!$exists){
   echo "<CENTER>No se han encontrado resultados del filtro ''$nombre'' + ''".$sigla."'' + ''".$acronimo."''</CENTER>";
	}
  }//IF PRINSIPAL
   ?>
   </div>
